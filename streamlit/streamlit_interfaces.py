...
if portal == "Client Portal":
    ...
    if st.button("Deposit"):
        if contract.functions.isUser(account_address).call({'from': account_address}) == True:
            try:
                ...
                st.warning(
                    "Unable to process deposit. Please verify that your wallet has enough funds")
        else:
    ...
    if st.button("Withdraw"):
        if contract.functions.isUser(account_address).call({'from': account_address}) == True:
            try:
                ...
                st.error(
                    "Not enough funds in your reserve account to cover the requested withdraw amount. Contact your advisor.")
        else:
    ...
    if st.button("Check Company Wallet Balance"):
        ...
        st.info(
            f"Company Balance: {company_balance:,} Wei ({convert_to_eth(company_balance):.4f} ETH)")

    if st.button("Check Contract Balance"):
        contract_balance = contract.functions.getContractBalance().call(
            {'from': company_account})
        st.info(
            f"Contract Balance: {contract_balance:,} Wei ({convert_to_eth(contract_balance):.4f} ETH)")
    ...
    if st.button("Display Client Data"):
        if contract.functions.isUser(client_address).call({'from': company_account}) == True:
            ...
            st.write(
                f"Balance: {balance} Wei ({convert_to_eth(balance):.4f} ETH))")
    ...
    if st.button("Deposit Funds To Client Account"):
        if contract.functions.isUser(client_address).call({'from': company_account}) == True:
            if contract.functions.getCompanyBalance().call(
                ...
                contract_balance = contract.functions.getContractBalance().call(
                    {'from': company_account})
                client_balance = contract.functions.getUser(
                    client_address).call({'from': client_address})[4]
                st.success(
                    f"The remaining company Balance is: {company_balance:,} Wei ({convert_to_eth(company_balance):.4f} ETH)")
                st.success(
                    f"The current contract balance is {contract_balance:,} Wei ({convert_to_eth(contract_balance):.4f} ETH)")
                st.success(
                    f"The client's balance is {client_balance:,} Wei ({convert_to_eth(client_balance):.4f} ETH)\n")
            else:
                st.error(
                    "Not enough funds in company wallet to cover deposit amount requested.")
        else:
        ...
        if contract.functions.isUser(client_address).call({'from': company_account}) == True:
            if contract.functions.getUser(client_address).call({'from': client_address})[4] >= company_amount:
                contract.functions.companyWithdrawal(
                    client_address, company_amount).transact({'from': company_account})
                ...
                client_balance = contract.functions.getUser(
                    client_address).call({'from': client_address})[4]
                st.success(
                    f"The new company balance is: {company_balance:,} Wei ({convert_to_eth(company_balance):.4f} ETH)")
                st.success(
                    f"The current contract balance is {contract_balance:,} Wei ({convert_to_eth(contract_balance):.4f} ETH)")
                st.success(
                    f"The client's balance is {client_balance:,} Wei ({convert_to_eth(client_balance):.4f} ETH)\n")
                ...
                st.error(
                    "Not enough funds in client account to cover requested withdrawal")
        else:
    ...
    if st.button("Update Client"):
        if contract.functions.isUser(c_address).call({'from': company_account}) == False:
            st.warning(
                "Client does not exist. Please use 'Client Add' to add a new client.")
        else:
            contract.functions.updateUser(
                c_address, c_lname, c_fname, c_email, c_portfolio).transact({"from": company_account})
