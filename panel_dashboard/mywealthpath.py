...
template = FastListTemplate(
    title="MyWealthPath", header_background='blue', favicon="images/favicon.ico")
...
def main_display(_):
    ...
    df_port_cum_returns, df_market_cum_returns, portfolio_returns, market_daily_returns = hst.get_cum_returns(
        stocks, market, weights)
...
# grabbing contents from modules/intro.py file and assembling into panel panes
    ...
    tabs_pane = pn.pane.PNG(
        "https://drive.google.com/uc?id=1RuqJmAkdxuNSkFoDk4OB6Qi_JIsGgLlD", width=400)
    portfolios_intro_pane = pn.pane.Markdown(portfolios_intro_text)
    portfolios_pane = pn.pane.PNG(
        "https://drive.google.com/uc?id=1lr_nc7ayQNyIvSJll5E61f4cZZPokKlK", width=1000)
...
##########
    ...
    compare_chart = hst.make_comparison_chart(
        df_port_cum_returns, df_market_cum_returns, port_class_text)
...
##########
    ...
    async def change_pane(event):
        ...
        if (mc_button.clicks == 1):
            ...
            simulation_plot, distribution_plot, summary, text = MCTab.prep_MC_data(
                stocks, weights)

            distribution_pane = pn.pane.Matplotlib(distribution_plot, dpi=144)
            summary_pane = pn.pane.DataFrame(
                summary.to_frame(name='statistics'))
    ...
    at_strategy_one, at_strategy_two, at_strategy_three, at_strategy_four, at_strategy_five, at_strategy_six = at.get_strategy_options(
        port_class_text)
    ...
    strategy_button = pn.widgets.MenuButton(
        name='Select Strategy', items=menu_items, button_type='primary')
    ...
    async def strategy_selected(event):
        ...
        at_fig, at_rio, at_compare = at.get_performance_data(
            port_class_text, event.new)
        ...
        filepath = Path(
            f"./MCdata/mcItems_{event.new}_{port_class_text}.joblib")
        ...
        selected investment strategy for the {port_class_text} portfolio showing potential returns over the next 10 years. Please refer to the
        ...
        at_simulation_plot = pn.pane.PNG(
            Path(f"./figures/simulation_{event.new}_{port_class_text}.png"))
        at_distribution_plot = pn.pane.PNG(
            Path(f"./figures/distribution_{event.new}_{port_class_text}.png"))
    ...
    return pn.Tabs(("Introduction", pn.Column(intro_pane, portfolios_intro_pane, portfolios_pane, disclaimer_pane)),
                   ...
                   ("Alogrithmic Trading", pn.Column(pn.Row(at_intro_pane),
                                                     ...
                                                     pn.Row(at_info_pane),
                                                     pn.Row(
                                                         pn.Column(at_graph_row1, at_graph_row2)),
                                                     # pn.Row(at_roi_info),
                                                     pn.Row(
                                                         pn.Column(at_compare_row1, at_compare_row2)),
                                                     pn.Row(at_mc_row1),
                                                     pn.Row(at_mc_row2),
                                                     pn.Row(at_mc_row3)))
